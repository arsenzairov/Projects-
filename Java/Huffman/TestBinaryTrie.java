...
public class TestBinaryTrie {
    ...
    public void bestPrefixMatchWithExactMatches() {
        ...
        BitSequence shouldBeC = new BitSequence("011010001");
        m = trie.longestPrefixMatch(shouldBeC);
        assertEquals((char) 'c', (char) m.getSymbol());
        assertEquals(new BitSequence("01"), m.getSequence());

        BitSequence shouldBeB = new BitSequence("001010001");
        m = trie.longestPrefixMatch(shouldBeB);
